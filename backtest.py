...
def train_model_at_date(df, cutoff_date):
    # 1. Filter for PAST games only (Strict < Cutoff)
...
def run_backtest():
    ...
    # We set the finish line to exactly 00:00:00 TODAY.
    # This ensures we grade everything UP TO last night, but touch nothing from today.
    
    # 1. Get "Midnight Today" (e.g., 2026-01-08 00:00:00)
    end_date = pd.Timestamp.now().normalize()
    
    # 2. Safety Filter: Drop any rows in the dataframe that are >= end_date
    # This guarantees no "Today" data exists in the loop even if the CSV has it.
    df = df[df['date'] < end_date].copy()

    start_date = end_date - timedelta(weeks=WEEKS_BACK)
    
    print(f"   -> Testing Range: {start_date.date()} up to {end_date.date()} (Midnight)")
    print(f"      (This strictly includes Yesterday, but excludes Today)")
    ...
    while current_date < end_date:
        ...
        # Cap the window at end_date so we don't overshoot
        if next_week > end_date:
            next_week = end_date
            
        # Train on EVERYTHING strictly before current window starts
